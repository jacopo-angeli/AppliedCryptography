...
atr = channel.getATR()
if atr == [0x3B,0x6A,0x00,0x00,0x09,0x44,0x31,0x31,0x43,0x52,0x02,0x00,0x25,0xC3]:
    print("[+] Feitian FT-Java/D11CR")
elif atr == [0x3B,0xFE,0x18,0x00,0x00,0x80,0x31,0xFE,0x45,0x80,0x31,0x80,0x66,0x40,0x90,0xA5,0x10,0x2E,0x10,0x83,0x01,0x90,0x00,0xF2]:
...
c = c[2:]
s = now()
r = send([0x00, 0x08, p1, p2, len(c)] + list(c)) # omit Le as a workaround for OMNIKEY 1021 and Feitian-D11CR T=0 bug
